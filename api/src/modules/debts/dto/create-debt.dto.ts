...
export class CreateDebtDto {
  ...
  @IsDateString(
    ...
    {
      message:
        'A data de vencimento deve estar no formato correto (YYYY-MM-DD).',
    },
  )
  dueDate!: Date;

  @ApiProperty({
    example: 'Dívida do cartão',
  ...
  @ApiProperty({
    example: 500,
    description: 'Valor atual da dívida',
    minimum: 0,
  })
  ...
  @IsOptional()
  @IsEnum(DebtStatus, {
    message: 'Status deve ser um dos seguintes: pendente, pago, em negociação.',
  })
  status?: DebtStatus;

  @ApiProperty({
    example: DebtType.CREDIT_CARD,
    enum: DebtType,
  ...
  @IsOptional()
  @IsEnum(DebtType, {
    message: 'Tipo de dívida deve ser "LOAN", "CREDIT_CARD" ou "PERSONAL"',
  })
  debtType?: DebtType;
}
